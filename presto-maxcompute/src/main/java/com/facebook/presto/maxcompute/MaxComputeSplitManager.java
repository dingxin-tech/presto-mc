...
public class MaxComputeSplitManager
    ...
    public ConnectorSplitSource getSplits(
        ...
        try {
            ...
            TableIdentifier tableIdentifier = TableIdentifier.of(table.getProject(), table.getSchemaName(), table.getName());
            TableReadSessionBuilder tableReadSessionBuilder =
                new TableReadSessionBuilder().identifier(tableIdentifier)
            ...
            if (projectedColumns.isPresent()) {
                projectedColumns.get().stream().map(e -> ((MaxComputeColumnHandle) e).getName()).forEach(dataColumns::add);
            }
            List<PartitionSpec> partitionSpecs = new ArrayList<>();
            if (table.isPartitioned()) {
                partitionSpecs = extractPartition(table, tableHandle.getConstraint());
                tableReadSessionBuilder.requiredPartitions(partitionSpecs);
            }
            log.info(String.format("Create TableReadSession, table %s, dataColumns %s, partitionSpec %s", tableIdentifier, dataColumns, partitionSpecs));
            ...
            List<MaxComputeSplit> splits = Arrays.stream(readSession.getInputSplitAssigner().getAllSplits()).map(e -> {
                ...
                return new MaxComputeSplit(tableIdentifier, maxComputeInputSplit, readSession, Collections.emptyMap());
